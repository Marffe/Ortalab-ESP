...
extern MY_HIGHP_OR_MEDIUMP vec2 kopi;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //vec2 rotater = vec2(cos(kopi.r*0.1221), sin(kopi.r*0.3512));


    if (kopi.g > 0.0 || kopi.g < 0.0) {
        hsl.x = 0.11;
        hsl.y *= hsl.y * 1.2;
        // hsl.z *= (1 - adjusted_uv.x*(cos(kopi.r*0.512)));
        // hsl.z *= (1 - adjusted_uv.y*(cos(kopi.r*0.512)));
    }
    if (hsl.z < 0.4) {
        hsl.x = 0.055;
        hsl.y = 0.51;
