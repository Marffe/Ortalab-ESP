...
extern MY_HIGHP_OR_MEDIUMP vec2 shinku;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    //vec2 rotater = vec2(cos(shinku.r*0.1221), sin(shinku.r*0.3512));


    if (shinku.g > 0.0 || shinku.g < 0.0) {
        hsl.x = 0.95;
        hsl.y *= hsl.y * 1.2;
        // hsl.z *= (1 - adjusted_uv.x*(cos(shinku.r*0.512)));
        // hsl.z *= (1 - adjusted_uv.y*(cos(shinku.r*0.512)));
    }
    if (hsl.z < 0.4) {
        hsl.x = 0.975;
        hsl.y = 0.51;
