...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords  )
{
    ...
    MY_HIGHP_OR_MEDIUMP number new_pixel_angle = atan(uv.y, uv.x) + (0.2 + 0.4*min(1.,speed))*uv_len - 0.1 -  speed*0.05 - min(1.,speed)*speed*0.06 + vort_offset;
    ...
    sv *= 42.;
    speed = time*(4.)*vort_speed + vort_offset + 1003.;
	MY_HIGHP_OR_MEDIUMP vec2 uv2 = vec2(sv.x+sv.y);

    for(int i=0; i < 5; i++) {
		uv2 += sin(max(sv.x, sv.y)) + sv;
		sv  += 0.5*vec2(cos(12.1123314 + 0.353*uv2.y - speed*0.131121),sin(uv2.x - 0.113*speed));
    ...
    MY_HIGHP_OR_MEDIUMP number c1p = max(0.,1. - 2.*abs(1.-smoke_res))/2.4;
    MY_HIGHP_OR_MEDIUMP number c2p = max(0.,1. - 2.*(smoke_res))/1.4;
